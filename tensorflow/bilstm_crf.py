...
class Model:
    ...
    def _build_net(self):
        ...
        '''
        print 'bilstm out = ', bilstm_out
        bilstm out =  Tensor("bilstm_crf/Tanh:0", shape=(32, 60, 11), dtype=float32) # (batch_size, sent_length, tag_size)
        
        '''

        '''
        print 'self.labels =', self.labels
        self.labels = Tensor("labels:0", shape=(32, 60), dtype=int32) # (batch_size, sent_length)
        '''
        ...
        3、句子长度: [60]*32
        '''
        log_likelihood, self.transition_params, self.x = tf.contrib.crf.crf_log_likelihood(bilstm_out, self.labels, tf.tile(np.array([self.sen_len]),np.array([self.batch_size])))
