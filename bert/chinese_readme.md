...
下面链接了带全字屏蔽的预培训模型。数据和训练完全相同，模型的结构和声乐与原始模型完全相同。
我们只包括bert大模型。使用这些模型时，请在论文中明确说明您使用的是bert large的全词屏蔽变体。
...
对于中文，我们用基于字，对于其他语言，用wordPiece的。这些模型都不需要改变你的代码（直接换模型就可以）。 
